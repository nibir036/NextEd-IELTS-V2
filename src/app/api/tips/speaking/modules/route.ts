...
/**
 * GET /api/tips/speaking/modules
 * Lists published Speaking Tips & Tricks modules with chapter summaries
 * and the current user's progress. Separate from /api/grammar — this
 * content surfaces inside Speaking Practice, not the Grammar LMS.
 */
export async function GET() {
  const userId = await getSessionUserId();

  const modules = await prisma.tips_modules.findMany({
    where: { skill: 'speaking', is_published: true },

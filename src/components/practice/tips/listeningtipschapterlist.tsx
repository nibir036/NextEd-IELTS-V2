...
export const ListeningTipsChapterList: React.FC<{
  ...
  useEffect(() => {
    ...
    fetch('/api/tips/listening/modules')
      .then(async (res) => {
        const data = await res.json();

        if (!res.ok) {
          throw new Error(data.error || 'Could not load Listening Tips & Tricks.');
        }
        ...
        if (!cancelled) {
          setError(
            err instanceof Error
              ? err.message
              : 'Could not load Listening Tips & Tricks. Please try again.',
          );
        }
      });
  ...
  if (modules.length === 0) {
    return (
      <GlassPanel className="p-8 text-center text-sm text-[var(--text-dim)]">
        Listening Tips &amp; Tricks content is being prepared. Please check back
        soon.
      </GlassPanel>
    );
  }

...
export const ListeningTipsReader: React.FC<{
  slug: string;
  ...
  useEffect(() => {
    ...
    fetch(`/api/tips/listening/chapters/${slug}`)
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
        ...
        // Fire-and-forget progress ping; ignore failures
        // (for example, when the user is logged out).
        fetch(`/api/tips/listening/chapters/${slug}/progress`, {
          method: 'POST',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button
          onClick={onBack}
          className="text-xs font-medium text-[var(--text-dim)] hover:text-[var(--text)] flex items-center gap-1"
        >
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        >
          {chapter.prev
            ? chapter.prev.title.replace(/^File \d+:\s*/, '')
            : 'First file'}
        </Button>
        ...
        >
          {chapter.next
            ? chapter.next.title.replace(/^File \d+:\s*/, '')
            : 'Last file'}
        </Button>
      </div>
    </div>

...
interface Lesson {
  ...
  position: number;
  difficulty?: number;
  // intro/introBn come from body; API may or may not include them
  body?: { intro?: string; introBn?: string };
  intro?: string;
  introBn?: string;
}

// Difficulty → band badge label (tweak to match your grammar cards)
...
export default function SpeakingTipsList() {
  ...
  useEffect(() => {
    async function fetchTips() {
      try {
        ...
        const speakingTips = (data.lessons || data || [])
          .filter((l: Lesson) => l.position >= 24 && l.position <= 29)
          .sort((a: Lesson, b: Lesson) => a.position - b.position);

        setLessons(speakingTips);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {lessons.map((lesson) => {
        ...
        return (
          <div
            key={lesson.id}
            className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 hover:border-amber-500/40 transition"
          >
            ...
            <Link
              ...
              className="mt-auto pt-5 flex items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] py-2.5 text-sm font-medium text-[var(--text)] hover:border-amber-500/40 hover:bg-amber-500/5 transition"
            >
              Start Speaking Tips

          ...
          <GlassPanel className="p-6 md:p-8 border border-[var(--border)] shadow-lg space-y-6 flex-1 flex flex-col justify-center">
            ...
            <div className="p-5 rounded-2xl bg-[var(--panel-2)]/90 border border-[var(--border)] space-y-4">
              ...
              <div className="flex items-baseline justify-between">
                <div>
                  <div className="text-xs text-[var(--text-faint)]">Current Evaluated</div>
                ...
                <div className="text-right">
                  <div className="text-xs text-[var(--text-faint)]">Target Band</div>
                  <div className="font-display font-extrabold text-2xl xl:text-3xl text-[var(--text)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
          {[
            ...
            },
            {
              id: 'listening',
              name: 'Listening',
              subtitle: 'Audio Play · 4 Sections',
              icon: Headphones,
              from: '#eab308',
              to: '#92400e',
            },
            ...
            return (
              <Reveal key={item.id} delayMs={idx * 120}>
                <div
                  onClick={() => onNavigateAction(item.id)}
                  className="relative overflow-hidden rounded-3xl p-4 cursor-pointer group aspect-[4/5] flex flex-col justify-between shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl"
                  style={{ background: `linear-gradient(160deg, ${item.from}, ${item.to})` }}
                >
                  {/* Ambient glow blob */}
                  <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                    <div className="w-24 h-24 rounded-full bg-white/10 blur-2xl group-hover:bg-white/20 transition-colors" />
                  </div>

                  <div className="relative flex items-center justify-between">
                    <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-white/20 text-white backdrop-blur-sm">
                      Ai
                    </span>
                  </div>

                  <div className="relative flex-1 flex items-center justify-center">
                    <Icon size={44} className="text-white/90 drop-shadow-lg group-hover:scale-110 transition-transform" />
                  </div>

                  <div className="relative rounded-2xl bg-white/15 backdrop-blur-md px-3 py-2.5">
                    <div className="font-display font-extrabold text-white text-base leading-tight">
                      {item.name}
                    </div>
                    <div className="text-[11px] text-white/80 mt-0.5 flex items-center gap-1">
